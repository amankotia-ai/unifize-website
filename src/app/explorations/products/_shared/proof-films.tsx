...
/* ============================================================================
 * proof-films.tsx - the shared Customer proof film rail (the treatment from
 * /explorations/products/dms/stylized). One lead card carrying the page's
 * single customer-attested figure, then the REAL customer films selected for
 * the page from the Website Customer Videos mirror (customer-films.ts).
...
import type { CustomerFilm, ProofLead } from "./customer-films";

export function ProofFilmRail({
  idPrefix,
  eyebrowN = 6,
  heading,
  lede,
  ...
  const trackId = `${idPrefix}-film-track`;
  ...
  const leadBody = lead && (
    <>
      <span className="dms-film__brand">{lead.label}</span>
      <div className="dms-film__result">
        <span className="dms-film__stat dms-data">{lead.stat}</span>
        <span className="dms-film__stat-lab">{lead.statLabel}</span>
      </div>
      <p className="dms-film__quote">{lead.body}</p>
      <span className="dms-film__meta">{lead.footnote}</span>
    </>
  );
      ...
      <div className="dms-wrap">
        <header className="dms-films__head">
          <div className="dms-head">
            ...
            <h2 className="dms-h2" id={`${idPrefix}-proof-title`}>{heading}</h2>
            <p className="dms-lede">{lede}</p>
          </div>

          <div className="dms-films__controls">
            <span className="dms-films__count">{String(films.length).padStart(2, "0")} {countNoun}</span>
        ...
        >
          {/* the one attested number, kept apart from the films */}
          {lead && (lead.href ? (
            <a className="dms-film dms-film--stat" href={lead.href} target="_blank" rel="noreferrer">
              {leadBody}
            </a>
          ) : (
          ...
          {films.map((f) => (
            <a
              ...
              aria-label={`Watch: ${f.title} — ${f.person}${f.company ? `, ${f.company}` : ""} (${f.duration})`}
            >
              ...
              <div className="dms-film__body">
                <span className="dms-film__tags">{(f.tags.length ? f.tags : [f.industry]).filter(Boolean).join(" · ")}</span>
                ...
                <div className="dms-film__foot">
                  <div className="dms-film__who">
                    ...
                    <span className="dms-film__meta">{f.role ? `${f.role} · ` : ""}{f.company ?? f.industry ?? ""}</span>
                  </div>
