...
/* ----------------------------------------------------------------------------
 * SiteHeader (Unifize enterprise design system) — sticky site header with the
 * real top-level nav: Platform · Products · Industries · About us · Resources.
 * Products, Industries and Resources open rich mega-panels (CSS hover/focus, so
 * no gap/flicker issues):
 ...
 *
 * Content-aware (ported from the DMS header): the header samples whichever
 * section sits directly beneath it and picks up that surface's palette, so bg,
 * logo, and nav swap in lock-step and it reads as a continuation of the section,
 * not a floating bar. Three themes:
 *   - dark  → hero, .itm-section--dark, the close, the footer
 *   - alt   → .itm-section--alt (light grey)
 ...
 * itm-ingress is the near-black mid-page block (personas · coverage · triggers);
 * it also catches the pinned sub-nav, which is not itself a section.
 * sft is the shared mega footer. */
const DARK_SURFACES = ["itm-hero", "itm-section--dark", "itm-ingress", "itm-close", "itm-footer", "sft"];
...
function PopCard({ x }: { x: NavLink }) {
  return (
    <a className="itm-header__card" href={x.href} role="menuitem">
      {x.icon ? <span className="itm-header__card-ic"><Glyph name={x.icon} /></span> : null}
      <span className="itm-header__card-tx">
        {x.code ? <span className="itm-header__card-code">{x.code}</span> : null}
        <span className="itm-header__card-title">{x.label}</span>
        {x.desc ? <span className="itm-header__card-desc">{x.desc}</span> : null}
      </span>
    </a>
  );
}
...
function PopFoot({ foot }: { foot: NavFoot }) {
  return (
    <a className="itm-header__pop-foot" href={foot.href} role="menuitem">
      <span className="itm-header__foot-tx">
        <span className="itm-header__foot-title">{foot.title}</span>
        <span className="itm-header__foot-desc">{foot.desc}</span>
      </span>
      <span className="itm-header__foot-cta">{foot.cta} {ARROW}</span>
    </a>
  );
}
...
function Pop({ item }: { item: NavItem }) {
  if (item.cols) {
    return (
      <div className={"itm-header__pop itm-header__pop--" + item.menu} role="menu">
        <div className="itm-header__pop-body">
              ...
              <div className="itm-header__col" key={col.heading}>
                ...
                {col.items.map((x) => (
                  <a className="itm-header__row" href={x.href} role="menuitem" key={x.label}>
                    ...
                    {x.desc ? <span className="itm-header__row-desc">{x.desc}</span> : null}
                  </a>
                ))}
              </div>
            ))}
  ...
  return (
    <div
      className={
        "itm-header__pop " +
        (isResources ? "itm-header__pop--resources" : "itm-header__pop--products")
      }
      role="menu"
    >
      <div className="itm-header__pop-body">
        <div className={"itm-header__pop-grid " + (isResources ? "itm-header__pop-grid--1" : "itm-header__pop-grid--2")}>
...
export function SiteHeader() {
  ...
  useEffect(() => {
    /* probe below the 68px header AND any pinned 56px ingress sub-nav, so we
     * read the section surface, not the sub-nav bar */
    const probeY = 130;
    const detect = () => {
  ...
  useEffect(() => {
    const place = () => {
      ...
      if (!vw) return;
      const gutter = 16;
      document.querySelectorAll<HTMLElement>(".itm-header__dd").forEach((dd) => {
        const pop = dd.querySelector<HTMLElement>(".itm-header__pop");
        if (!pop) return;
        ...
        pop.style.setProperty("--itm-pop-x", "0px");
      });
    };
    place();
